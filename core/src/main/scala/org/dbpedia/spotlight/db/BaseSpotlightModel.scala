...
class BaseSpotlightModel(val tokenizer: TextTokenizer,
                     ...
                     val relevanceScorer: Option[RelevanceScorer]) extends SpotlightModel{
...
object BaseSpotlightModel {
  ...
  def fromFolder(modelFolder: File): BaseSpotlightModel = {
    ...
    // properties.getProperty("relevance_scoring")
    def getRelevance(contextStore:ContextStore, scorerName:String):Option[RelevanceScorer] = scorerName match {
      case null => None
      case s: String if s equals "None" => None
      case s: String if s equals "default" =>  Some(new RelevanceScorer(contextStore, new ContextRelevanceScore()))
    }
    ...
    val relevanceScorer = getRelevance(contextStore, properties.getProperty("relevance_scoring"))
    val model = new BaseSpotlightModel(tokenizer, spotters, disambiguators, properties, relevanceScorer)
    model
  }
}
